...
// Deterministic calculations (no Math.random())
...
export function TechnicalIndicatorsPanel({ assetType, symbol, currentPrice, data = {} }: TechnicalIndicatorsPanelProps) {
  ...
  const supertrendColor = supertrend < currentPrice ? 'var(--accent-green)' : 'var(--accent-red)';

  const isBullish = rsi < 70 && adx > 25 && macd > 0;
  const biasColor = isBullish ? 'var(--accent-green)' : rsi > 70 ? 'var(--accent-red)' : 'var(--accent-gold)';
  const biasLabel = isBullish ? '✓ Bullish' : rsi > 70 ? '⚠ Overbought' : '↔ Neutral';
            ...
            {[
              ...
              <div key={label} style={{
                ...
                <span style={{
                  ...
                  fontWeight: 700,
                  color: colored
                    ? (val > 0 ? 'var(--accent-green)' : 'var(--accent-red)')
                    : 'var(--text-primary)',
                }}>
                  {val.toFixed(3)}
                </span>
