// ============================================================
// MULTI-SOURCE FALLBACKS: Yahoo v7 + BSE + Screener.in
// ============================================================
const _priceCache: Record<string, {price:number;change:number;ts:number}> = {};
const _cooldown: Record<string, number> = {};

async function fetchYahooQuoteV7(yahooSymbol: string): Promise<{price:number;change:number}|null> {
  try {
...
const BSE_MAP: Record<string,string> = {
  ...
  TITAN:'500114',ADANIENT:'512599',NTPC:'532555',ONGC:'500312',
  JSWSTEEL:'500228',TATASTEEL:'500470',BAJAJFINSV:'532978',
  TECHM:'532755',DRREDDY:'500124',CIPLA:'500087',
};

async function getBSEPrice(symbol: string): Promise<{price:number;change:number}|null> {
  const code = BSE_MAP[symbol]; if (!code) return null;
  const k = 'bse:' + symbol;
  if (_cooldown[k] && Date.now() < _cooldown[k]) return null;
  const c = _priceCache[k]; if (c && Date.now() - c.ts < 60000) return c;
  try {
    const url = `https://api.bseindia.com/BseIndiaAPI/api/getScripHeaderData/w?Debtflag=&scripcode=${code}&seriesid=`;
    const res = await fetch(url, { headers: { 'User-Agent':'Mozilla/5.0', Referer:'https://www.bseindia.com/', Accept:'application/json' },
      signal: (() => { const ac = new AbortController(); setTimeout(() => ac.abort(), 6000); return ac.signal; })() });
    if (res.status === 429 || res.status === 403) { _cooldown[k] = Date.now() + 300000; return null; }
    if (!res.ok) return null;
    const j: any = await res.json();
    const price = parseFloat(String(j?.CurrRate ?? j?.Ltp ?? '0').replace(/,/g,''));
    const change = parseFloat(String(j?.PcntChange ?? '0').replace(/,/g,'')) || 0;
    if (!Number.isFinite(price) || price <= 0) return null;
    _priceCache[k] = { price, change, ts: Date.now() }; return { price, change };
  } catch { _cooldown[k] = Date.now() + 120000; return null; }
}

async function getScreenerPrice(symbol: string): Promise<{price:number;change:number}|null> {
  const k = 'scr:' + symbol;
  if (_cooldown[k] && Date.now() < _cooldown[k]) return null;
  const c = _priceCache[k]; if (c && Date.now() - c.ts < 120000) return c;
  try {
    const url = `https://www.screener.in/company/${encodeURIComponent(symbol)}/consolidated/`;
    const res = await fetch(url, { headers: { 'User-Agent':'Mozilla/5.0 Chrome/120.0.0.0', Accept:'text/html' },
      signal: (() => { const ac = new AbortController(); setTimeout(() => ac.abort(), 10000); return ac.signal; })() });
    if (res.status === 429 || res.status === 403) { _cooldown[k] = Date.now() + 300000; return null; }
    if (!res.ok) return null;
    const html = await res.text();
    const top = html.match(/<ul id="top-ratios"[^>]*>([\s\S]*?)<\\/ul>/)?.[1] ?? "";
    const items = top.match(/<li[^>]*>([\\s\\S]*?)<\\/li>/g) ?? [];
    let price = 0;
    for (const item of items) {
      const t = item.replace(/<[^>]*>/g,' ').replace(/\\s+/g,' ').trim();
      if (t.includes('Current Price')) {
        const m = item.match(/<span class="number"[^>]*>([\d,]+(?:\.\d+)?)<\\/span>/);
        if (m) price = parseFloat(m[1].replace(/,/g,''));
      }
    }
    let change = 0;
    const up = html.match(/icon-circle-up[\\s\\S]{0,120}?([\\d.]+)%/);
    const dn = html.match(/icon-circle-down[\\s\\S]{0,120}?([\\d.]+)%/);
    if (up) change = parseFloat(up[1]) || 0;
    else if (dn) change = -(parseFloat(dn[1]) || 0);
    if (!Number.isFinite(price) || price <= 0) return null;
    _priceCache[k] = { price, change, ts: Date.now() }; return { price, change };
  } catch { _cooldown[k] = Date.now() + 120000; return null; }
}
