...
// Deterministic from stock data ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ¢¢"š¬‚ no Math.random()
function getRSI(asset: UniversalAsset): number {
  return Math.min(90, Math.max(10, ((asset.metadata?.roe || 10) * 2 + (asset.metadata?.opm || 10)) % 70 + 30));
}
function getADX(asset: UniversalAsset): number {
  return Math.min(80, Math.max(10, ((asset.metadata?.revcagr || 10) * 1.5 + (asset.metadata?.roce || 10)) % 60 + 20));
}
function getMACD(asset: UniversalAsset): number {
  return (((asset.metadata?.roe || 10) - (asset.metadata?.debt || 1) * 5) % 2) - 0.5;
}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
        ...
        <IndicatorCard title="RSI (14)">
          ...
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '8px' }}>
            {rsi > 70 ? 'ƒÆ’‚¢ƒ"¦‚¡ƒ"š‚  Overbought' : rsi < 30 ? 'ƒÆ’‚¢ƒ"¦¢‚¬Å“ƒ¢¢"š¬…" Oversold' : 'ƒÆ’‚¢ƒ¢¢"š¬‚ ƒ¢¢"š¬¢"ž¢ Neutral'}
          </div>
        </IndicatorCard>
        ...
        <IndicatorCard title="Supertrend">
          <div style={{ fontSize: '22px', fontWeight: 700, fontFamily: 'monospace', color: supertrend < asset.price ? 'var(--accent-green)' : 'var(--accent-red)' }}>
            {supertrend.toFixed(2)}
          </div>
          <div style={{ fontSize: '13px', marginTop: '8px', color: supertrend < asset.price ? 'var(--accent-green)' : 'var(--accent-red)', fontWeight: 600 }}>
            {supertrend < asset.price ? 'ƒÆ’‚¢ƒ¢¢"š¬¢‚¬Å“ƒ"š‚² UPTREND ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ¢¢"š¬‚ Price above Supertrend' : 'ƒÆ’‚¢ƒ¢¢"š¬¢‚¬Å“ƒ"š‚¼ DOWNTREND ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ¢¢"š¬‚ Price below Supertrend'}
          </div>
        </IndicatorCard>
        ...
        <IndicatorCard title="Technical Bias">
          <div style={{ fontSize: '20px', fontWeight: 700, color: rsi < 70 && adx > 25 && macd > 0 ? 'var(--accent-green)' : rsi > 70 ? 'var(--accent-red)' : 'var(--accent-gold)' }}>
            {rsi < 70 && adx > 25 && macd > 0 ? 'ƒÆ’‚¢ƒ"¦¢‚¬Å“ƒ¢¢"š¬…" Bullish' : rsi > 70 ? 'ƒÆ’‚¢ƒ"¦‚¡ƒ"š‚  Overbought' : 'ƒÆ’‚¢ƒ¢¢"š¬‚ ƒ¢¢"š¬¢"ž¢ Neutral'}
          </div>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '8px', lineHeight: 1.6 }}>
            RSI {rsi > 70 ? 'Overbought' : rsi < 30 ? 'Oversold' : 'Neutral'} ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ"š‚¢
            ADX {adx > 25 ? 'Strong' : 'Weak'} ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ"š‚¢
