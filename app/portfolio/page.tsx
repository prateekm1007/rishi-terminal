...
// Deterministic pseudo-random using seed (no Math.random in render)
...
function generateHistoricalData(holdings: any[], startDate: Date, endDate: Date) {
  ...
  for (let i = 0; i <= daysDiff; i += interval) {
    ...
    holdings.forEach((h, hi) => {
      ...
      const randomWalk = Math.exp(
        (drift - (volatility * volatility) / 2) * i + volatility * Math.sqrt(i + 1) * noise
      );
...
export default function PortfolioPage() {
  ...
  const handleAddHolding = () => {
    ...
    if (!symbol || !STOCKS[symbol])           { alert('Invalid stock symbol'); return; }
    if (isNaN(shares)   || shares   <= 0)     { alert('Shares must be positive'); return; }
    if (isNaN(avgPrice) || avgPrice <= 0)     { alert('Price must be positive'); return; }
  ...
  const handleWeightChange = (name: string, weight: number) => {
    if (weight < 0 || weight > 10) { alert('Weight must be 0-10'); return; }
  ...
  if (!portfolio) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-primary)' }}>
        <div style={{ color: 'var(--text-muted)', fontFamily: 'monospace', letterSpacing: 2 }}>LOADING PORTFOLIO...</div>
      </div>
    );
  }
  ...
  const chartData = portfolio.holdings.length > 0
    ? generateHistoricalData(portfolio.holdings, startDate, now)
    : [];
  ...
  const scoreBg = (s: number) =>
    s >= 75 ? 'rgba(0,186,124,0.15)' : s >= 55 ? 'rgba(255,215,0,0.15)' : 'rgba(244,33,46,0.15)';

  const SUB_TABS = [
    { id: 'holdings', label: 'Holdings',   desc: 'Your positions' },
    { id: 'returns',  label: 'My Returns', desc: 'Performance & CAGR' },
  ];
  ...
  return (
    <main className="page-container">
      ...
      <div className="page-header">
        <div className="content-wrapper">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 24, marginBottom: 28 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', maxWidth: 480, lineHeight: 1.6 }}>
                Track holdings and analyze performance through your custom Rishi lens.
              </p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 14 }}>
            {[
              { label: 'Invested',      value: formatCurrency(metrics.totalInvested),  color: 'var(--text-primary)' },
              { label: 'Current Value', value: formatCurrency(metrics.totalCurrent),   color: 'var(--text-primary)' },
              {
                label: 'Total Return',
              ...
              { label: 'Holdings', value: metrics.holdingsCount.toString(), color: 'var(--text-primary)' },
            ].map((m, i) => (
      ...
      <div className="content-wrapper" style={{ padding: '28px 24px' }}>
              ...
              <div className="card-sacred" style={{ padding: 0, overflow: 'hidden' }}>
                <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-primary)' }}>
                  <div className="philosophy-heading" style={{ fontSize: 13, letterSpacing: 2, color: 'var(--text-muted)' }}>
                    HOLDINGS — {metrics.holdingsCount} POSITION{metrics.holdingsCount !== 1 ? 'S' : ''}
                  </div>
                </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border-primary)', background: 'var(--bg-secondary)' }}>
                        {['Stock', 'Shares', 'Avg Price', 'Current', 'P&L', 'Return %', 'Rishi Score', ''].map((h, i) => (
                          <th key={i} style={{
                        ...
                        return (
                          ...
                          >
                            <td style={{ padding: '16px' }}>
                              ...
                              {h.notes && (
                                <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 3, fontStyle: 'italic', opacity: 0.7 }}>
                                  {h.notes}
                                </div>
                              )}
                            </td>
                            <td style={{ textAlign: 'right', padding: '16px', fontFamily: 'monospace', color: 'var(--text-primary)', fontSize: 13 }}>
                              {h.shares.toLocaleString()}
                            </td>
                            <td style={{ textAlign: 'right', padding: '16px', fontFamily: 'monospace', color: 'var(--text-secondary)', fontSize: 13 }}>
                              {h.avgPrice.toFixed(2)}
                            </td>
                            <td style={{ textAlign: 'right', padding: '16px', fontFamily: 'monospace', fontWeight: 700, color: 'var(--text-primary)', fontSize: 13 }}>
                            ...
                            <td style={{ textAlign: 'right', padding: '16px' }}>
                              <span style={{
                                ...
                                fontWeight: 700,
                                fontFamily: 'monospace',
                                background: scoreBg(consensus),
                                color: scoreColor(consensus),
                              }}>
                                {consensus}
                              </span>
                            </td>
                      ...
                      <tfoot>
                        <tr style={{ borderTop: '2px solid var(--border-primary)', background: 'var(--bg-secondary)' }}>
                          <td colSpan={3} style={{ padding: '14px 16px', fontSize: 11, color: 'var(--text-muted)', fontFamily: 'monospace', letterSpacing: 1 }}>
                            TOTAL ({metrics.holdingsCount} positions)
                          </td>
                          <td style={{ textAlign: 'right', padding: '14px 16px', fontFamily: 'monospace', fontWeight: 700, color: 'var(--text-primary)' }}>
        ...
        {/* ══════════════════════════════════
            TAB: MY RETURNS
            Performance chart + CAGR analysis
            ══════════════════════════════════ */}
        {activeTab === 'returns' && (
          <>
            {portfolio.holdings.length === 0 ? (
              <div className="card-sacred" style={{ padding: 48, textAlign: 'center' }}>
                <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12, letterSpacing: 1 }}>NO DATA YET</div>
                <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                  Add holdings to track your historical performance and CAGR.
                </p>
              </div>
            ) : (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
                  {[
                    ...
                    {
                      label: 'Current Value',
                      value: formatCurrency(metrics.totalCurrent),
                      sub: 'Mark to market',
                      color: 'var(--text-primary)',
                      big: false,
                    },
                    {
                      ...
                      value: `${metrics.totalGainLossPct >= 0 ? '+' : ''}${metrics.totalGainLossPct.toFixed(2)}%`,
                      sub: formatCurrency(Math.abs(metrics.totalGainLoss)),
                      color: metrics.totalGainLoss >= 0 ? 'var(--accent-green)' : 'var(--accent-red)',
                      big: false,
                    },
                  ].map((m, i) => (
                    <div key={i} className="card-sacred" style={{ padding: '20px 24px', position: 'relative', overflow: 'hidden' }}>
                      ...
                      <div style={{ fontSize: m.big ? 36 : 24, fontWeight: 700, fontFamily: 'monospace', color: m.color }}>
                        {m.value}
                      </div>
                ...
                <div className="card-sacred" style={{ padding: 24 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4, opacity: 0.7 }}>
                        Simulated based on current holdings & market drift
                      </div>
                    </div>
                  ...
                  <div style={{ display: 'flex', gap: 24, justifyContent: 'center', marginTop: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--text-muted)' }}>
                      <div style={{ width: 24, height: 2, background: 'var(--text-muted)', borderTop: '1px dashed var(--text-muted)' }} />
                      Cost Basis
                    </div>
                  </div>
                </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-secondary)' }}>
                        {['Stock', 'Invested', 'Current', 'Return %', 'Rishi Score'].map((h, i) => (
                          <th key={i} style={{
                        ...
                        .map(row => {
                          ...
                          return (
                            <tr key={h.symbol} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                              ...
                              <td style={{ textAlign: 'right', padding: '14px 16px', fontFamily: 'monospace', color: 'var(--text-secondary)', fontSize: 13 }}>
                                {formatCurrency(invested)}
                              </td>
                              <td style={{ textAlign: 'right', padding: '14px 16px', fontFamily: 'monospace', color: 'var(--text-primary)', fontSize: 13 }}>
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
          <div className="card-sacred" style={{ maxWidth: 480, width: '100%', padding: 28 }}>
            <h2 className="philosophy-heading" style={{ fontSize: 20, marginBottom: 20 }}>Add Holding</h2>
            {[
              { label: 'Symbol',        value: formSymbol,   set: setFormSymbol,   type: 'text',   placeholder: 'RELIANCE, TCS, HDFC...' },
              { label: 'Shares',        value: formShares,   set: setFormShares,   type: 'number', placeholder: '100' },
              { label: 'Avg Buy Price', value: formAvgPrice, set: setFormAvgPrice, type: 'number', placeholder: '2500.00', step: '0.01' },
            ].map((f, i) => (
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea
                placeholder="Why did you buy this? What is your thesis?"
                value={formNotes}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={handleAddHolding} style={{
                flex: 1, padding: '10px', background: 'var(--accent-gold)', color: '#000',
                border: 'none', borderRadius: 6, fontWeight: 700, fontSize: 13, cursor: 'pointer',
              }}>
                Add to Portfolio
              </button>
              <button
                onClick={() => { setShowAddModal(false); setFormSymbol(''); setFormShares(''); setFormAvgPrice(''); setFormNotes(''); }}
                style={{
      ...
      {showWeightsModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 24 }}>
          <div className="card-sacred" style={{ maxWidth: 680, width: '100%', padding: 28, maxHeight: '85vh', overflowY: 'auto' }}>
            <h2 className="philosophy-heading" style={{ fontSize: 20, marginBottom: 8 }}>Custom Rishi Weights</h2>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 20, lineHeight: 1.6 }}>
              Adjust weights to match your own investment philosophy. Changes apply to all consensus scoring.
            </p>
            <div style={{ marginBottom: 20 }}>
              {RISHI_WEIGHT_CONFIG.map(rishi => {
                ...
                return (
                  <div key={rishi.name} style={{
                    ...
                    {isCustom && (
                      <button
                        onClick={() => { removeCustomWeight(rishi.name); refreshPortfolio(); }}
                        style={{
                          padding: '4px 10px',
                          background: 'transparent',
                          color: 'var(--accent-red)',
            ...
            <button
              ...
              style={{
                width: '100%', padding: '10px', background: 'var(--accent-gold)', color: '#000',
                border: 'none', borderRadius: 6, fontWeight: 700, fontSize: 13, cursor: 'pointer',
              }}
            >
              Save & Close
