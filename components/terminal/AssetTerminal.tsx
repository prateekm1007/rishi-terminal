...
import { AssetMetricsPanel }      from './AssetMetricsPanel';
...
export function AssetTerminal({ asset, consensus, detail }: Props) {
  ...
  const TABS = [
    ...
    { id: 'technical', label: t('asset.technicals'), desc: t('asset.technicalsDesc') },
    { id: 'wisdom',    label: t('asset.rishiWisdom'), desc: t('asset.wisdomDesc')     },
  ];
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 24 }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 16, fontSize: 12, color: '#64748B', letterSpacing: 1, fontFamily: 'monospace' }}>
                <span>{asset.sector ?? asset.metadata?.sector ?? asset.category}</span>
                ...
                <div className="wisdom-reveal-delay-1">
                  {(() => {
                    ...
                    const metrics = {
                      ...
                      debtToEquity: asset.metadata?.de ?? 0,
                      revenueCAGR3Y: asset.metadata?.revcagr ?? 0,
                      epsCAGR3Y: asset.metadata?.epscagr ?? 0,
                      promoterHolding: asset.metadata?.promo ?? 0,
                      marketCap: asset.metadata?.mktcap ?? 0,
                      fcfMargin: (asset.metadata?.fcf && asset.metadata?.rev)
                        ? (asset.metadata.fcf / asset.metadata.rev) * 100
                        : 0,
                    };
