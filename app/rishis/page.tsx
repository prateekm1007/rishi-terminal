...
const ALL_RISHIS = [
  ...
  { id: 'pabrai', name: 'Mohnish Pabrai', emoji: '🎯', category: 'Stock', origin: 'Global', tier: 'Master', label: 'Dhandho Cloner', bio: 'Clones the best ideas from the best investors. Dhandho framework - high upside, low downside.', philosophy: 'Heads I win, tails I do not lose much. Clone shamelessly from the best.', formula: 'Clone Score (30%) + Owner-Operator (25%) + Downside Protection (25%) + Upside (20%)', bestFor: ['Cloning', 'Asymmetric', 'Value'], quote: 'Heads I win, tails I do not lose much.', famousPicks: ['Fiat Chrysler', 'Rain Industries'] },
...
const TIER_STYLES: Record<string, { bg: string; color: string }> = {
  ...
  Master:     { bg: 'rgba(29,155,240,0.15)',   color: '#3B82F6' },
...
export default function RishisPage() {
  ...
  const CATEGORIES = useMemo(() => [
    { id: 'All',       label: t('rishis.all')  },
    { id: 'Stock',     label: 'Stock'           },
    { id: 'Crypto',    label: t('nav.crypto')  },
    { id: 'Commodity', label: t('nav.commodities') },
    { id: 'Forex',     label: t('nav.forex')   },
  ], [t, locale]);
        ...
        <div className="content-wrapper">
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 24, marginBottom: 32 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', maxWidth: 600, lineHeight: 1.6 }}>
                {ALL_RISHIS.length} {t('rishis.subtitle')} {t('rishis.philosophyDesc')}
              </p>
            </div>

            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setViewMode('catalog')} style={{
                padding: '10px 20px',
          ...
          {viewMode === 'catalog' && (
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', marginBottom: 16 }}>
              <input
                type="text"
                placeholder={t('rishis.searchPlaceholder')}
                value={search}
                onChange={e => setSearch(e.target.value)}
                style={{
                  flex: 1, minWidth: 200, padding: '10px 16px', borderRadius: 6,
                  fontSize: 13, fontFamily: 'monospace', background: 'var(--bg-card)',
                ...
                {CATEGORIES.map(cat => (
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: activeCategory === cat.id ? 700 : 400, fontFamily: 'monospace',
                      border: activeCategory === cat.id ? 'none' : '1px solid var(--border-primary)',
                      background: activeCategory === cat.id
                        ? (cat.id === 'All' ? 'var(--accent-gold)' : CATEGORY_COLORS[cat.id] || 'var(--accent-gold)')
                        : 'var(--bg-card)',
                      color: activeCategory === cat.id ? '#000' : 'var(--text-muted)',
                      cursor: 'pointer',
                    }}
                  >
                    {cat.label} ({counts[cat.id as keyof typeof counts] ?? 0})
                  </button>
                ))}
              </div>
            </div>
        ...
        {viewMode === 'catalog' && (
          <>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 20, fontFamily: 'monospace' }}>
              {t('rishis.showing')} {filtered.length} {t('rishis.rishis')}
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {filtered.map(rishi => {
                const isExpanded = expandedId === rishi.id;
                const catColor   = CATEGORY_COLORS[rishi.category] || 'var(--accent-gold)';
                      ...
                      <div style={{ borderTop: '1px solid var(--border-primary)', padding: 20, background: 'var(--bg-secondary)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14, marginBottom: 16 }}>
                          <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: 16, border: '1px solid var(--border-subtle)' }}>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: 1, marginBottom: 8, fontFamily: 'monospace' }}>{t('rishis.biography').toUpperCase()}</div>
                            <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.7 }}>{rishi.bio}</p>
                          </div>
                          <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: 16, border: '1px solid var(--border-subtle)' }}>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: 1, marginBottom: 8, fontFamily: 'monospace' }}>{t('rishis.philosophy').toUpperCase()}</div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 }}>
                          <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: 16, border: '1px solid var(--border-subtle)' }}>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: 1, marginBottom: 8, fontFamily: 'monospace' }}>{t('rishis.formula').toUpperCase()}</div>
                            <p style={{ fontSize: 11, color: catColor, lineHeight: 1.7, fontFamily: 'monospace' }}>{rishi.formula}</p>
                          </div>
                          <div style={{ background: 'var(--bg-card)', borderRadius: 8, padding: 16, border: '1px solid var(--border-subtle)' }}>
                            <div style={{ fontSize: 10, color: 'var(--text-muted)', letterSpacing: 1, marginBottom: 8, fontFamily: 'monospace' }}>{t('rishis.famousPicks').toUpperCase()}</div>
            ...
            {filtered.length === 0 && (
              <div style={{ textAlign: 'center', padding: '80px 0', color: 'var(--text-muted)' }}>
                ...
                <p style={{ fontSize: 14 }}>{t('rishis.noMatch')}</p>
              </div>
            )}
          </>
        )}
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}>
                  {[
                    ...
                    { sym: stockB, stock: STOCKS[stockB], consensus: consensusB, color: '#3B82F6' },
                    ...
                    <div key={sym} className="card-sacred" style={{ padding: 20, borderTop: '3px solid ' + color }}>
                      ...
                      {consensus.scores.slice(0, 10).map(r => (
                        <div key={r.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                          ...
                          <div style={{ flex: 1, height: 18, background: 'var(--bg-secondary)', borderRadius: 2, overflow: 'hidden' }}>
                            <div style={{ width: r.score + '%', height: '100%', background: r.score >= 75 ? '#00BA7C' : r.score >= 55 ? '#FFD700' : '#F4212E', borderRadius: 2 }} />
                          </div>
                      ...
                      <Link href={'/stock/' + sym} style={{
                        display: 'inline-block', marginTop: 12, padding: '6px 14px', borderRadius: 4,
                        background: color + '15', border: '1px solid ' + color + '40', color,
                        fontSize: 11, textDecoration: 'none', fontWeight: 600, fontFamily: 'monospace',
                      }}>
                        {t('rishis.deepDive')} {'>'}
                      </Link>
                    </div>
                ...
                <div className="card-sacred" style={{ padding: 20 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
                    {consensusA.scores.map((rA, i) => {
                      ...
                      const winColor = winner === stockA ? 'var(--accent-gold)' : winner === stockB ? '#3B82F6' : 'var(--text-muted)';
                      return (
                        <div key={rA.name} style={{
                          background: 'var(--bg-secondary)', borderRadius: 6, padding: 12,
                          borderLeft: '3px solid ' + winColor, border: '1px solid var(--border-subtle)',
                        }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                            ...
                            <span style={{ fontSize: 11, fontFamily: 'monospace', color: winner === stockB ? '#3B82F6' : 'var(--text-muted)' }}>
                              {stockB}: {rB.score}
                            </span>
                          </div>
                          <div style={{ fontSize: 10, color: winColor, fontWeight: 700, textAlign: 'center', padding: '4px', background: winColor + '15', borderRadius: 3, fontFamily: 'monospace' }}>
                            {winner === 'TIE' ? t('rishis.tied').toUpperCase() : winner + ' ' + t('rishis.wins').toUpperCase()}
                          </div>
                        </div>
                      );
                    })}
