...
/**
 * Deterministic template selection based on score
 * Prevents hydration mismatches by avoiding Math.random()
 */
function pick(arr: string[], score: number): string {
  const index = Math.floor(score) % arr.length;
  return arr[index];
}

export function generateCommentary(
  result: RishiScoreResult,
  metrics: StockMetrics
): string {
  const { conviction, mode, finalScore } = result;

  if (mode === "LONG") {
    if (conviction === "LEGENDARY") return pick(LEGENDARY_LONG, finalScore);
    ...
    if (conviction === "HIGH_CONVICTION_SHORT") return pick(HIGH_CONVICTION_SHORT, finalScore);
    return pick(TACTICAL_SHORT, finalScore);
  }
}
