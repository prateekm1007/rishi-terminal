// components/terminal/AssetPriceChart.tsx
// TOOLTIP_CHART_V2
'use client';
...
export function AssetPriceChart({ asset }: Props) {
  ...
  const { points, loading, error, source } = usePriceHistory(asset?.symbol ?? '', tf);
  if (!asset || !asset.price) return null;

  const first  = points[0]?.v ?? asset.price;
  const last   = points[points.length-1]?.v ?? asset.price;
