...
function DebateCard({
  ...
  const icon = side === 'bull' ? 'ƒÆ’‚°ƒ"¦‚¸ƒ"š‚ƒ¢¢"š¬…¡' : 'ƒÆ’‚°ƒ"¦‚¸ƒ"š‚ƒ"š‚Â»';
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: '#F8FAFC' }}>
            {play.rishi} ƒÆ’‚¢ƒ¢¢"š¬‚ ƒ¢¢"š¬¢"ž¢ {play.stock}
          </div>
          <div style={{ fontSize: 11, color: '#64748B', marginTop: 2 }}>
            {play.yearBought} ƒÆ’‚¢ƒ¢¢‚¬Å¡‚¬ƒ"š‚¢ {play.market}
          </div>
        </div>
        {play.return && (
          <div style={{
...
function getRelevantPlays(asset: UniversalAsset): RishiPlay[] {
  ...
  const stockSectorLower = (asset.sector ?? asset.metadata?.sector ?? asset.category).toLowerCase();
  const keywords = SECTOR_KEYWORDS[asset.sector ?? ''] || [stockSectorLower];
  ...
  const scored = GLOBAL_RISHI_PLAYS.map(play => {
    ...
    if (play.market === 'India' && asset.metadata?.exchange === 'NSE') score += 3;

    // Category match (quality stocks ƒÆ’‚¢ƒ¢¢"š¬‚ ƒ¢¢"š¬¢"ž¢ quality plays)
    if (asset.metadata?.roe >= 20 && play.category === 'quality') score += 4;
    if (asset.metadata?.pe < 15 && play.category === 'value') score += 4;
    if (asset.metadata?.revcagr >= 20 && play.category === 'growth') score += 4;
...
function getRelevanceNote(play: RishiPlay, asset: UniversalAsset): string {
  const playLower = play.stock.toLowerCase();
  const sectorLower = (asset.sector ?? asset.metadata?.sector ?? '').toLowerCase();

  if (playLower.includes(sectorLower) || play.thesis.toLowerCase().includes(sectorLower)) {
    return `${asset.sector} sector parallel - study ${play.rishi}'s approach`;
  }

  if (play.market === 'India' && asset.metadata?.exchange === 'NSE') {
    return `Indian market success story by ${play.rishi}`;
  }

  if (play.category === 'quality' && asset.metadata?.roe >= 20) {
    return `Quality business playbook - similar ROE profile to ${asset.symbol}`;
  }

  if (play.category === 'value' && asset.metadata?.pe < 15) {
    return `Value investing case study - similar valuation approach`;
  }

  if (play.category === 'growth' && asset.metadata?.revcagr >= 20) {
    return `High-growth opportunity - similar to ${asset.symbol}'s trajectory`;
  }
...
export function KnowledgeGraphView({ asset, consensus }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      const data = buildEliteKnowledgeGraph(asset as any, consensus.scores);
      setGraphData(data);
    } catch (e) {
  ...
  const relevantPlays = getRelevantPlays(asset);
        ...
        {activeView === 'debate' && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <DebateCard
                side="bull"
                rishi={consensus.topBull?.name ?? 'Buffett'}
                argument={consensus.topBull?.insight ?? 'Strong fundamentals support long-term value.'}
                score={consensus.topBull?.score ?? 80}
                color="#22C55E"
              />
              <DebateCard
                side="bear"
                rishi={consensus.topBear?.name ?? 'Chanos'}
                argument={consensus.topBear?.insight ?? 'Elevated valuation creates downside risk.'}
                ...
                {graphData.debate.bulls.length > 0 && (
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#22C55E", letterSpacing: "0.1em", marginBottom: 12 }}>
                      ƒÆ’‚°ƒ"¦‚¸ƒ"š‚ƒ¢¢"š¬…¡ BULL ARGUMENTS ({graphData.debate.bulls.length})
                    </div>
                ...
                {graphData.debate.bears.length > 0 && (
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#EF4444", letterSpacing: "0.1em", marginBottom: 12 }}>
                      ƒÆ’‚°ƒ"¦‚¸ƒ"š‚ƒ"š‚Â» BEAR ARGUMENTS ({graphData.debate.bears.length})
                    </div>
                ...
                {graphData.debate.neutrals.length > 0 && (
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#D4AF37", letterSpacing: "0.1em", marginBottom: 12 }}>
                      ƒÆ’‚¢ƒ"¦‚¡ƒ¢¢"š¬¢‚¬Å“ƒÆ’‚¯ƒ"š‚¸ƒ"š‚ NEUTRAL POSITIONS ({graphData.debate.neutrals.length})
                    </div>
        ...
        {activeView === 'historical' && (
          <div>
            <div style={{ fontSize: 11, color: '#64748B', marginBottom: 16, lineHeight: 1.6 }}>
              ...
              <strong style={{ color: '#F8FAFC' }}>{asset.name}</strong>.
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {relevantPlays.map((play, i) => (
                <PlayCard
                  key={i}
                  play={play}
                  relevanceNote={getRelevanceNote(play, asset)}
                />
              ))}
            </div>
          </div>
